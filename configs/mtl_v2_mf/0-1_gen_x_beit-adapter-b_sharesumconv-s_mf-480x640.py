...
model = dict(
    ...
    decode_head=dict(
        ...
        num_queries=100,
        # for pseudo-x generator
        # ----------------------------------------
        num_bins=256,
        task_max_val=1,
        task_min_val=0,
        task_conv_cfg=dict(type='Conv'),
        gen_x_pixel_loss_weight=0.1,
        # ----------------------------------------
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(vit_num_layers=12, decay_rate=0.9))
...
default_hooks = dict(
    ...
    visualization=dict(type='MTLVisualizationHook', interval=1, draw=True))
...
vis_backends = [dict(type='LocalVisBackend'),
                # dict(type='WandbVisBackend', init_kwargs=dict(project="ECCV-MFNet", name="mtl_ta_x_convnext-adapter-b-s_layer_decay_095_lr_2e-5")),
]
visualizer = dict(

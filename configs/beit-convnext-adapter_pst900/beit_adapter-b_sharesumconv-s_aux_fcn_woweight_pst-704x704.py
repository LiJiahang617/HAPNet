# dataset settings
dataset_type = 'MMPST900Dataset'
data_root = '/media/ljh/Kobe24/PST900_RGBT_Dataset/PST900_RGBT'

# vit-adapter needs square, so crop must has h==w
crop_size = (704, 704) # h, w
img_size = (704, 1280) # h, w
...
train_pipeline = [
    ...
    dict(type='RandomResize', scale=(1280, 704),
...
val_pipeline = [
    ...
    dict(
        type='Resize',
        scale=(1280, 704), keep_ratio=True),  # Note: w, h instead of h, w
...
test_pipeline = [
    ...
    dict(type='LoadMFImageFromFile', to_float32=True, modality='thermal'),
    dict(type='StackByChannel', keys=('img', 'ano')),
    dict(type='Resize', scale=(1280, 704), keep_ratio=True),
    dict(type='LoadAnnotations', reduce_zero_label=False),
    dict(type='PackSegInputs')
]
...
model = dict(
    ...
    train_cfg=dict(),
    test_cfg=dict(mode='slide', crop_size=crop_size, stride=(640, 640))) #h,w
...
vis_backends = [dict(type='LocalVisBackend'),
                # dict(type='WandbVisBackend', init_kwargs=dict(project="RTFormer-PST900", name="beit-b_share_sum_convnext-s_ld_090_lr1e-4_aux_fcn")),
]
visualizer = dict(

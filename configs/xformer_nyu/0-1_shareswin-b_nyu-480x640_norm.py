...
# for RGB encoder
custom_keys.update({
    f'backbone.stages.{stage_id}.blocks.{block_id}.norm_x': backbone_norm_multi
    for stage_id, num_blocks in enumerate(depths)
    for block_id in range(num_blocks)
})
# for another encoder
custom_keys.update({
    f'backbone.stages.{stage_id}.blocks.{block_id}.norm_y': backbone_norm_multi
    for stage_id, num_blocks in enumerate(depths)
    for block_id in range(num_blocks)
})
custom_keys.update({
...
vis_backends = [dict(type='LocalVisBackend'),
                # dict(type='WandbVisBackend', init_kwargs=dict(project="RoadFormer_nyu-480x640", name="shareswin-b_0-1_hha-3ch_norm")),
]
visualizer = dict(

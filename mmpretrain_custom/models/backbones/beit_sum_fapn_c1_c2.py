...
from mmcv_custom.ops.modulated_deform_conv import ModulatedDeformConv2dMask
from mmengine_custom.model import BaseModule, ModuleList, Sequential
from mmengine_custom.model.weight_init import (constant_init, trunc_normal_, caffe2_xavier_init, xavier_init,
                                               normal_init)
...
class BEiTAdapter_rgbxsum_fapn_c1_c2_relu(BEiT):
    arch_settings = {
        'atto': {
            'depths': [2, 2, 6, 2],
    ...
    def __init__(self, pretrain_size=224, n_points=4, deform_num_heads=6,
                 ...
                 interaction_indexes=None, add_vit_feature=True, with_cp=False,
        ...
        # replace transposeconv with FaPN
        ...
        # feature align normally use GN:32 in this situation
        align_norm_cfg = dict(type='GN', num_groups=32)
        use_bias = align_norm_cfg == None
        self.lateral_conv_c1 = ConvModule(embed_dim, embed_dim, kernel_size=1, stride=1,
                                          bias=use_bias, norm_cfg=align_norm_cfg, act_cfg=None)
        self.align_c1_c2 = FeatureAlign(embed_dim, embed_dim, align_norm_cfg)
        # TODO: consider if relu is ok here may do ablation study here
        print('Init output_conv_c1 with RELU activation')
        self.output_conv_c1 = ConvModule(embed_dim, embed_dim, kernel_size=3, stride=1, padding=1,
                                         bias=use_bias, norm_cfg=align_norm_cfg, act_cfg=dict(type='ReLU'))
    ...
    def forward(self, x):
        ...
        # implement FaPN here
        cur_fpn = self.lateral_conv_c1(c1)
        # Following FPN implementation, we use nearest upsampling here
        c1 = self.output_conv_c1(self.align_c1_c2(cur_fpn, c2))
        # c1 = c1 + self.up(c2)

        if self.add_vit_feature:
            x1, x2, x3, x4 = outs
            # first try apply fapn in one stage (mask feature)

...
class BEiTAdapter_spmsum(BEiT):
    ...
    def __init__(self, pretrain_size=224, conv_inplane=64, n_points=4, deform_num_heads=6,
        ...
        # spm related modules
        self.fc1 = nn.Conv2d(embed_dim, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc2 = nn.Conv2d(embed_dim, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc3 = nn.Conv2d(embed_dim, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc4 = nn.Conv2d(embed_dim, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)

        self.spm_norm1x = nn.LayerNorm(embed_dim)
        self.spm_norm2x = nn.LayerNorm(embed_dim)
        self.spm_norm3x = nn.LayerNorm(embed_dim)
        self.spm_norm4x = nn.LayerNorm(embed_dim)
        self.spm_norm1y = nn.LayerNorm(embed_dim)
        self.spm_norm2y = nn.LayerNorm(embed_dim)
        self.spm_norm3y = nn.LayerNorm(embed_dim)
        self.spm_norm4y = nn.LayerNorm(embed_dim)
    ...
    def forward(self, x):
        ...
        c2_x = self.spm_norm2x(c2_x)
        c3_x = self.spm_norm3x(c3_x)
        c4_x = self.spm_norm4x(c4_x)

        c2_y = self.spm_norm2x(c2_y)
        c3_y = self.spm_norm3x(c3_y)
        c4_y = self.spm_norm4x(c4_y)
        # keep feature channels same
        # TODO: try test if fc helps bridging attention and conv to get better performance
        c1 = c1_x+c1_y
        c2 = c2_x+c2_y
        c3 = c3_x+c3_y
        c4 = c4_x+c4_y

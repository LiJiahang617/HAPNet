...
class BEiTAdapter_patch_thermal_alone_mpm_rgb_alone(BEiT):
    arch_settings = {
        'atto': {
            'depths': [2, 2, 6, 2],
    ...
    def __init__(self, pretrain_size=224, n_points=4, deform_num_heads=6,
        ...
        print('Note, this is ablation study for adapter-thermal_alone_patch_rgb_alone_mpm!!!')
    ...
    def forward(self, inputs):
        ...
        c1, c2, c3, c4 = self.x_modality_encoder(torch.cat((x,x), dim=1))
        ...
        for i, layer in enumerate(self.interactions):
            ...
            y, c, cls = layer(y, c, cls, self.blocks[indexes[0]:indexes[-1] + 1],
                              deform_inputs1, deform_inputs2, H, W)
            # need different context level features for segmentation
            outs.append(y.transpose(1, 2).view(bs, dim, H, W).contiguous())

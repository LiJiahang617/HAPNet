...
class DualBEiT(BaseBackbone):
    ...
    def __init__(self, img_size=512, patch_size=16, in_chans=3,
        ...
        if hybrid_backbone is not None:
            self.patch_embed_x = HybridEmbed(hybrid_backbone, img_size=img_size,
                                           in_chans=in_chans, embed_dim=embed_dim)
            self.patch_embed_y = HybridEmbed(hybrid_backbone, img_size=img_size,
                                           in_chans=in_chans, embed_dim=embed_dim)
        else:
            self.patch_embed_x = PatchEmbed(img_size=img_size, patch_size=patch_size,
                                          in_chans=in_chans, embed_dim=embed_dim)
            self.patch_embed_y = PatchEmbed(img_size=img_size, patch_size=patch_size,
                                            in_chans=in_chans, embed_dim=embed_dim)
        ...
        if use_shared_rel_pos_bias:  # False not use
            self.rel_pos_bias = RelativePositionBias(window_size=self.patch_embed_x.patch_shape, num_heads=num_heads)
        else:
        ...
        self.blocks_x = nn.ModuleList([
            Block(
                ...
                init_values=init_values, window_size=self.patch_embed_x.patch_shape if use_rel_pos_bias else None)
        ...
        if self.init_cfg is None:
            ...
            for key in all_keys:
                ...
                if 'relative_position_bias_table' in key:
                    cur_key = key.replace('blocks.', 'blocks_x.')
                    rel_pos_bias = _state_dict[key]
                    src_num_pos, num_attn_heads = rel_pos_bias.size()
                    dst_num_pos, _ = self.state_dict()[cur_key].size()
                    dst_patch_shape = self.patch_embed_x.patch_shape
                    ...
                    if src_size != dst_size:
                        ...
                        new_rel_pos_bias = torch.cat((rel_pos_bias, extra_tokens),
                                                     dim=0)
                        _state_dict[key.replace('blocks.', 'blocks_x.')] = new_rel_pos_bias
                        _state_dict[key.replace('blocks.', 'blocks_y.')] = new_rel_pos_bias

            state_dict = OrderedDict()
            for k, v in _state_dict.items():
                if k.startswith('backbone.'):
                    if 'patch_embed.' in k:
                        k_x = k.replace('patch_embed.', 'patch_embed_x.')
                        k_y = k.replace('patch_embed.', 'patch_embed_y.')
                        state_dict[k_x[9:]] = v
                        state_dict[k_y[9:]] = v
                    else:
                        state_dict[k[9:]] = v

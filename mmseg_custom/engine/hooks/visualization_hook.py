...
@HOOKS.register_module()
class GanVisualizationHook(Hook):
    """Fake images generated by joint learning in Seg-Gan. Used to visualize validation and
    testing process prediction results.
        ...
        if self.every_n_inner_iters(batch_idx, self.interval):
            for output in outputs:
                ...
                self._visualizer.add_datasample(
                    ...
                    wait_time=self.wait_time,
                    step=runner.iter)
                self._visualizer.add_rgbxgansample(
                    window_name,
                    data_sample=output,
                    show=self.show,
                    wait_time=self.wait_time,
...
class MTLVisualizationHook(Hook):
    """Task-aware images generated by joint learning in multi-task framework. Used to visualize validation and
    testing process prediction results.
        ...
        if self.every_n_inner_iters(batch_idx, self.interval):
            for output in outputs:
                ...
                self._visualizer.add_datasample(
                    ...
                    wait_time=self.wait_time,
                    step=runner.iter)
                self._visualizer.add_mtltasample(
                    window_name,
                    data_sample=output,
                    show=self.show,
                    wait_time=self.wait_time,

...
@TRANSFORMS.register_module()
class LoadKPAnnotations(MMCV_LoadAnnotations):
    """Load annotations for KP day-night semantic segmentation dataset.
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        gt_semantic_seg = mmcv_custom.imfrombytes(
            img_bytes, flag='unchanged',
            backend=self.imdecode_backend).squeeze().astype(np.uint8)
        # KP dataset do not use the ``void`` category.
        gt_semantic_seg[gt_semantic_seg == 19] = 255
        # reduce zero_label
        if self.reduce_zero_label is None:
            self.reduce_zero_label = results['reduce_zero_label']
        assert self.reduce_zero_label == results['reduce_zero_label'], \

...
class PhotoMetricDistortion(BaseTransform):
    ...
    def convert(self,
        ...
        modified by Kobe Li for handling the multi-modal situation.
...
class MultiModalPhotoMetricDistortion(BaseTransform):
    """Apply photometric distortion to multi-modal image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last.
    ...
    def convert(self,
        ...
        # multi-modal seg
            ...
            img[:, :, :3] = img[:, :, :3].astype(np.float32) * alpha + beta
            img = np.clip(img, 0, 255)
        else:
    ...
    def convert_for_saturation(self,
                img: np.ndarray,
                alpha: int = 1,
                beta: int = 0) -> np.ndarray:
        """Multiple with alpha and add beat with clip.
    ...
    def saturation(self, img: np.ndarray) -> np.ndarray:
        ...
        if random.randint(2):
            rgb_channels = mmcv_custom.bgr2hsv(rgb_channels)
            rgb_channels[:, :, 1] = self.convert_for_saturation(
                rgb_channels[:, :, 1],
                alpha=random.uniform(self.saturation_lower,
                                     self.saturation_upper))
            rgb_channels = mmcv_custom.hsv2bgr(rgb_channels)
        if c > 3:
            # concat back
            img = np.concatenate((rgb_channels, x_channels), axis=2)
        else:
            img = rgb_channels
    ...
    def hue(self, img: np.ndarray) -> np.ndarray:
        ...
        if random.randint(2):
            rgb_channels = mmcv_custom.bgr2hsv(rgb_channels)
            rgb_channels[:, :, 0] = (rgb_channels[:, :, 0].astype(int) +
                                     random.randint(-self.hue_delta, self.hue_delta)) % 180
            rgb_channels = mmcv_custom.hsv2bgr(rgb_channels)

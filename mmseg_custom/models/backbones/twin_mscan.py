...
class TwinMSCAN(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(num_stages):
            if i == 0:
                patch_embed_x = StemConv(3, embed_dims[0], norm_cfg=norm_cfg)
                patch_embed_y = StemConv(3, embed_dims[0], norm_cfg=norm_cfg)
            else:
                patch_embed_x = OverlapPatchEmbed(
                    patch_size=7 if i == 0 else 3,
                    stride=4 if i == 0 else 2,
                    in_channels=in_channels if i == 0 else embed_dims[i - 1],
                    embed_dim=embed_dims[i],
                    norm_cfg=norm_cfg)
                patch_embed_y = OverlapPatchEmbed(
                    patch_size=7 if i == 0 else 3,
                    stride=4 if i == 0 else 2,
                    in_channels=in_channels if i == 0 else embed_dims[i - 1],
                    embed_dim=embed_dims[i],
            ...
            norm_x = nn.LayerNorm(embed_dims[i])
            norm_y = nn.LayerNorm(embed_dims[i])
            cur += depths[i]
            # RGB encoder
            setattr(self, f'patch_embed_x{i + 1}', patch_embed_x)
            setattr(self, f'block_x{i + 1}', block_x)
            setattr(self, f'norm_x{i + 1}', norm_x)
            # another encoder
            setattr(self, f'patch_embed_y{i + 1}', patch_embed_y)
            setattr(self, f'block_y{i + 1}', block_y)
            setattr(self, f'norm_y{i + 1}', norm_y)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            ...
            block_x = getattr(self, f'block_x{i + 1}')
            norm_x = getattr(self, f'norm_x{i + 1}')
            x, H, W = patch_embed_x(x)
            for blk_x in block_x:
                x = blk_x(x, H, W)
            x = norm_x(x)
        ...
        # another encoder
        for i in range(self.num_stages):
            patch_embed_y = getattr(self, f'patch_embed_y{i + 1}')
            block_y = getattr(self, f'block_y{i + 1}')
            norm_y = getattr(self, f'norm_y{i + 1}')
            y, H, W = patch_embed_y(y)
            for blk_y in block_y:
                y = blk_y(y, H, W)
            y = norm_y(y)

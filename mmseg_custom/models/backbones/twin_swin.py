...
from mmcv_custom.cnn.bricks.transformer import FFN, build_dropout
...
class TwinSwinTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(num_layers):
            if i < num_layers - 1:
                downsample_x = PatchMerging(
                    in_channels=in_channels,
                    out_channels=2 * in_channels,
                    stride=strides[i + 1],
                    norm_cfg=norm_cfg if patch_norm else None,
                    init_cfg=None)
                downsample_y = PatchMerging(
                    in_channels=in_channels,
                    out_channels=2 * in_channels,
                    stride=strides[i + 1],
                    norm_cfg=norm_cfg if patch_norm else None,
                ...
                downsample_x = None
                downsample_y = None
            # one stage in rgb encoder
            stage_x = SwinBlockSequence(
                ...
                downsample=downsample_x,
                act_cfg=act_cfg,
                norm_cfg=norm_cfg,
                with_cp=with_cp,
                init_cfg=None)
            # one stage in ano encoder
            stage_y = SwinBlockSequence(
                embed_dims=in_channels,
                num_heads=num_heads[i],
                feedforward_channels=int(mlp_ratio * in_channels),
                depth=depths[i],
            ...
            if downsample_x and downsample_y:
                assert downsample_x.out_channels == downsample_y.out_channels, f'x and y have different out channels,' \
                                                                               f'please check stage{i}!'
                in_channels = downsample_x.out_channels

        self.num_features = [int(embed_dims * 2**i) for i in range(num_layers)]
        # Add a norm layer for each output
        for i in out_indices:
            layer_x = build_norm_layer(norm_cfg, self.num_features[i])[1]
            layer_x_name = f'norm_x{i}'
            self.add_module(layer_x_name, layer_x)
            layer_y = build_norm_layer(norm_cfg, self.num_features[i])[1]
            layer_y_name = f'norm_y{i}'
            self.add_module(layer_y_name, layer_y)
    ...
    def _freeze_stages(self):
        if self.frozen_stages >= 0:
            ...
            for param in self.patch_embed_y.parameters():
                param.requires_grad = False
            if self.use_abs_pos_embed:
                self.absolute_pos_embed_x.requires_grad = False
                self.absolute_pos_embed_y.requires_grad = False
            self.drop_after_pos.eval()

        for i in range(1, self.frozen_stages + 1):

            if (i - 1) in self.out_indices:
                norm_layer_x = getattr(self, f'norm_x{i - 1}')
                norm_layer_x.eval()
                norm_layer_y = getattr(self, f'norm_y{i - 1}')
                norm_layer_y.eval()
                for param in norm_layer_x.parameters():
                    param.requires_grad = False
                for param in norm_layer_y.parameters():
                    param.requires_grad = False

            m_x = self.stages_x[i - 1]
            m_x.eval()
            for param in m_x.parameters():
                param.requires_grad = False
            m_y = self.stages_y[i - 1]
            m_y.eval()
            for param in m_y.parameters():
                param.requires_grad = False
    ...
    def forward(self, x):
        ...
        y, yhw_shape = self.patch_embed_y(y)

        if self.use_abs_pos_embed:
            x = x + self.absolute_pos_embed_x
        ...
        for i, (stage_x, stage_y) in enumerate(zip(self.stages_x, self.stages_y)):
            x, xhw_shape, out_x, out_xhw_shape = stage_x(x, xhw_shape)
            y, yhw_shape, out_y, out_yhw_shape = stage_y(y, yhw_shape)
            if i in self.out_indices:
                norm_layer_x = getattr(self, f'norm_x{i}')
                norm_layer_y = getattr(self, f'norm_y{i}')
                out_x = norm_layer_x(out_x)
                out_y = norm_layer_y(out_y)
                out_x = out_x.view(-1, *out_xhw_shape, self.num_features[i]).permute(0, 3, 1, 2).contiguous()

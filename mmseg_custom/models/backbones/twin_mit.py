...
from mmcv_custom.cnn import Conv2d, build_activation_layer, build_norm_layer
...
class EfficientMultiheadAttention(MultiheadAttention):
    ...
    def __init__(self,
        ...
        from mmseg_custom import digit_version, mmcv_version
...
class TwinMixVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.layers_y = ModuleList()
        for i, num_layer in enumerate(num_layers):
            embed_dims_i = embed_dims * num_heads[i]
            patch_embed_x = PatchEmbed(
                in_channels=in_channels,
            ...
            # The ret[0] of build_norm_layer is norm name.
            norm_x = build_norm_layer(norm_cfg, embed_dims_i)[1]
            norm_y = build_norm_layer(norm_cfg, embed_dims_i)[1]
            # RGB encoder
            self.layers_x.append(ModuleList([patch_embed_x, layer_x, norm_x]))
            # another encoder
            self.layers_y.append(ModuleList([patch_embed_y, layer_y, norm_y]))
            cur += num_layer
    ...
    def forward(self, x):
        ...
        for i, layer_x in enumerate(self.layers_x):
            x, hw_shape = layer_x[0](x)
            for block_x in layer_x[1]:
                x = block_x(x, hw_shape)
            x = layer_x[2](x)
            x = nlc_to_nchw(x, hw_shape)
            if i in self.out_indices:
                outs_x.append(x)
        # another encoder
        for i, layer_y in enumerate(self.layers_y):
            y, hw_shape = layer_y[0](y)
            for block_y in layer_y[1]:
                y = block_y(y, hw_shape)
            y = layer_y[2](y)
            y = nlc_to_nchw(y, hw_shape)
            if i in self.out_indices:
                outs_y.append(y)

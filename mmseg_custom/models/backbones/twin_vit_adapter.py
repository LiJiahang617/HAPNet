    ...
    def __init__(
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed = PatchEmbed(
                ...
                in_chans=in_channels,
                embed_dim=embed_dim)
        # TODO: test if separate patch embedding has better performance
...
class InteractionBlockWithCls(nn.Module):
    ...
    def __init__(self,
        ...
        if extra_extractor:
            self.extra_extractors_x = nn.Sequential(*[
                Extractor(
                    dim=dim,
                    num_heads=num_heads,
                    n_points=n_points,
            ...
            self.extra_extractors_y = nn.Sequential(*[
                Extractor(
                    dim=dim,
                    num_heads=num_heads,
                    n_points=n_points,
            ...
            self.extra_extractors_x = None
            self.extra_extractors_y = None

    def forward(self, x, y, c_x, c_y, cls_x, cls_y, blocks, deform_inputs1, deform_inputs2, H, W):
        x = self.injector_x(
            query=x,
            reference_points=deform_inputs1[0],
            feat=c_x,
        ...
        if self.extra_extractors_x is not None:
            for extractor_x in self.extra_extractors_x:
                c_x = extractor_x(
                    query=c_x,
                    reference_points=deform_inputs2[0],
                    feat=x,
                    ...
                    H=H,
                    W=W)
        if self.extra_extractors_y is not None:
            for extractor_y in self.extra_extractors_y:
                c_y = extractor_y(
                    query=c_y,
                    reference_points=deform_inputs2[0],
                    feat=y,
        ...
        return x, y, c_x, c_y, cls_x, cls_y
...
class TwinBEiTAdapter_sharespm(BEiT):
    """
    Multi-modal twin beit-adapter implementation for rgb-x segmentation
    Share weights in ViT branch, spm branch, but separate cls_tokens, injector and extractor and
    final add to construct feats.
    """
    ...
    def __init__(self,
        ...
        self.norm1_x = nn.SyncBatchNorm(embed_dim)
        self.norm2_x = nn.SyncBatchNorm(embed_dim)
        self.norm3_x = nn.SyncBatchNorm(embed_dim)
        self.norm4_x = nn.SyncBatchNorm(embed_dim)

        self.norm1_y = nn.SyncBatchNorm(embed_dim)
        self.norm2_y = nn.SyncBatchNorm(embed_dim)
        self.norm3_y = nn.SyncBatchNorm(embed_dim)
        self.norm4_y = nn.SyncBatchNorm(embed_dim)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.interactions):
            ...
            x, y, c_x, c_y, cls_x, cls_y = layer(x, y, c_x, c_y, cls_x, cls_y,
        ...
        # Final Norm
        f1 = self.norm1_x(c1_x) + self.norm1_y(c1_y)
        f2 = self.norm2_x(c1_x) + self.norm2_y(c1_y)
        f3 = self.norm3_x(c1_x) + self.norm3_y(c1_y)
        f4 = self.norm4_x(c1_x) + self.norm4_y(c1_y)

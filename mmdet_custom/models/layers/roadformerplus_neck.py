...
class RoadFormerplusNeck(BaseModule):
    ...
    def __init__(self,
        ...
        # transpose conv
        self.up = nn.ConvTranspose2d(feat_channels, feat_channels, 2, 2)
        self.up_norm = nn.GroupNorm(num_groups=32, num_channels=feat_channels)
    ...
    def init_weights(self) -> None:
        """Initialize weights."""
        print('Start init pixel decoder now!')
        ...
        caffe2_xavier_init(self.up, bias=0)
        constant_init(self.up_norm, 1, bias=0)
        ...
        for m in self.encoder.layers.modules():
            if isinstance(m, MultiScaleDeformableAttention):
                m.init_weights()
        print('End init pixel decoder now!')
    def forward(self, feats: List[Tensor]) -> Tuple[Any, List[Any]]:
        ...
        # transpose conv
        outs[-1] = outs[-1] + self.up(outs[-2])
        outs[-1] = self.up_norm(outs[-1])
